...
### Acceptance Criteria
- [ ] Dictionary auto-downloads on first use
- [ ] Downloaded data cached at `~/.cache/vocab/`
- [ ] Supports multiple source languages (fr, de, es, it, pt)
- [ ] `lookup()` returns `DictionaryEntry` for known words
- [ ] `lookup()` returns `None` for unknown words
- [ ] `lookup()` populates target_translations when target_language provided
- [ ] IPA correctly extracted from Wiktionary data
- [ ] Gender correctly extracted for nouns
- [ ] English translations always extracted
